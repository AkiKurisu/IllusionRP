...
{
    // Reference: UnityEngine.Rendering.HighDefinition.LightCookieManager (rectangle area light slice)
    internal sealed class AreaLightCookieManager
    {
        ...
        static readonly ProfilingSampler s_AreaLightCookieConvolutionSampler = new("Area Light Cookie Convolution");
        ...
        public AreaLightCookieManager(Shader filterAreaLightCookiesPS, int cookieAtlasSize, GraphicsFormat cookieFormat, int cookieAtlasLastValidMip)
        {
            // Also make sure to create the engine material that is used for the filtering
            m_MaterialFilterAreaLights = CoreUtils.CreateEngineMaterial(filterAreaLightCookiesPS);

            this.cookieFormat = cookieFormat;
            this.cookieAtlasLastValidMip = cookieAtlasLastValidMip;

            m_CookieAtlas = new PowerOfTwoTextureAtlas(cookieAtlasSize, cookieAtlasLastValidMip, cookieFormat, name: "Area Light Cookie Atlas", useMipMap: true);
        }
        ...
        Texture FilterAreaLightTexture(CommandBuffer cmd, Texture source, int finalWidth, int finalHeight)
        {
            ...
            using (new ProfilingScope(cmd, s_AreaLightCookieConvolutionSampler))
            {
        ...
        public void LayoutIfNeeded()
        {
            ...
            {
                Debug.LogError($"No more space in the 2D Cookie Texture Atlas. To solve this issue, increase the resolution of the area light cookie atlas in the Illusion Renderer Feature.");
                m_NoMoreSpace = true;
            }
        }
        ...
        public Vector4 FetchAreaCookie(CommandBuffer cmd, Texture cookie)
        {
            ...
            if (!m_CookieAtlas.IsCached(out var scaleBias, cookie) && !m_NoMoreSpace)
                Debug.LogError($"Area Light cookie texture {cookie} can't be fetched without having reserved. You can try to increase the area light cookie atlas resolution in the Illusion Renderer Feature.");
